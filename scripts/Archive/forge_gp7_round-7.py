...
class ForgingClient:
  ...
  def __init__(self):
    ...
    self.furnace_pick.position.z = 0.3965
  ...
  def relieve_corrected_forces(self):
    ...
    dist_increment = .0005
    move_speed = 0.01
    force_score_multiplier = 1 / upper_force_threshold
    torque_score_multiplier = 1 / upper_torque_threshold
    ...
    while 1 in move_list or -1 in move_list:
      ...
      if (ft_sensor.corrected.wrench.force.x < -lower_force_threshold) and x_success:
        x_dir = -1
      elif (ft_sensor.corrected.wrench.force.x > lower_force_threshold) and x_success:
        x_dir = 1
      else:
        x_dir = 0
      if (ft_sensor.corrected.wrench.force.y < -lower_force_threshold) and y_success:
        y_dir = -1
      elif (ft_sensor.corrected.wrench.force.y > lower_force_threshold) and y_success:
        y_dir = 1
      else:
        y_dir = 0
      if (ft_sensor.corrected.wrench.force.z < -lower_force_threshold) and z_success:
        z_dir = -1
      elif (ft_sensor.corrected.wrench.force.z > lower_force_threshold) and z_success:
        z_dir = 1
      else:
        z_dir = 0
      ...
      # Make moves in X-direction if needed, then check whether successful at relieving forces and roll back if unsuccessful
      if x_dir != 0:
        target_pose.position.x += (x_dir*dist_increment)
        rospy.loginfo("X move to relieve forces. Target: x = {}".format(target_pose.position.x))
        ...
        move_score = force_score_multiplier*((abs(fx0) - abs(ft_sensor.corrected.wrench.force.x)) + (abs(fy0) - abs(ft_sensor.corrected.wrench.force.y)) + (
          abs(fz0) - abs(ft_sensor.corrected.wrench.force.z))) + torque_score_multiplier*((abs(tx0) - abs(ft_sensor.corrected.wrench.torque.x)) + (
          abs(ty0) - abs(ft_sensor.corrected.wrench.torque.y)) + (abs(tz0) - abs(ft_sensor.corrected.wrench.torque.z)))

        x_success = move_score > 0

        if x_success == False:
          # Last move had a negative overall effect. Flip sign of move direction and execute to move back to previous position.
          x_dir *= -1
          target_pose.position.x += (x_dir*dist_increment)
          rospy.loginfo("Previous X move to relieve forces made things worse. Moving back. Target: x = {}".format(target_pose.position.x))
          try:
            self.move_scanning_object_to_pose(target_pose, move_speed)
          except Exception as err:
            print(err.args)
      ...
      # Make moves in Y-direction if needed, then check whether successful at relieving forces and roll back if unsuccessful
      if y_dir != 0:
        target_pose.position.y += (y_dir*dist_increment)
        rospy.loginfo("Y move to relieve forces. Target: y = {}".format(target_pose.position.y))
        ...
        y_success = move_score > 0

        if y_success == False:
          # Last move had a negative overall effect. Flip sign of move direction and execute to move back to previous position.
          y_dir *= -1
          target_pose.position.y += (y_dir*dist_increment)
          rospy.loginfo("Previous Y move to relieve forces made things worse. Moving back. Target: y = {}".format(target_pose.position.y))
          try:
            self.move_scanning_object_to_pose(target_pose, move_speed)
          except Exception as err:
            print(err.args)
      ...
      # Make moves in Z-direction if needed, then check whether successful at relieving forces and roll back if unsuccessful
      if z_dir != 0:
        target_pose.position.z += (z_dir*dist_increment)
        rospy.loginfo("Z move to relieve forces. Target: z = {}".format(target_pose.position.z))
        ...
        z_success = move_score > 0

        if z_success == False:
          # Last move had a negative overall effect. Flip sign of move direction and execute to move back to previous position.
          z_dir *= -1
          target_pose.position.z += (z_dir*dist_increment)
          rospy.loginfo("Previous Z move to relieve forces made things worse. Moving back. Target: z = {}".format(target_pose.position.z))
          try:
            self.move_scanning_object_to_pose(target_pose, move_speed)
          except Exception as err:
            print(err.args)
    ...
    if abs(ft_sensor.corrected.wrench.force.x) < lower_force_threshold and abs(ft_sensor.corrected.wrench.force.y) < lower_force_threshold and abs(ft_sensor.corrected.wrench.force.z) < lower_force_threshold:
      rospy.loginfo("No forces to relieve")
    elif (abs(fx) > upper_force_threshold) or (abs(fy) > upper_force_threshold) or (
...
def main():
  ...
  while press_schedule.keep_pressing:
    ...
    for index, value in enumerate(zip(press_schedule.poses.poses, press_schedule.hitMags, press_schedule.minZs, press_schedule.maxZs)):
      ...
      # REMOVE THESE LINES!!!!!!
      pose.orientation.x = 0.3825
      pose.orientation.w = 0.9240
      ############################
