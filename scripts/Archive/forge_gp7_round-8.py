...
class PressSchedule():
  ...
  def call_D3D(self):
    program_filepath = "/home/capsen/AIMS_D3D/DREAM3D-2023_1_25/DREAM3D-6.6.275.364368cf9-Linux-x86_64/bin/PipelineRunner"
  ...
  def write_files(self, scan_counter):
    ...
    self.hdf5_filename = "/home/capsen/ws_forge/src/D3D_planning/hitSequence_" + str(scan_counter) + ".hdf5"
    new_d3d = "/home/capsen/ws_forge/src/D3D_planning/FormingDemo_Part1_Iter_" + str(scan_counter) + ".dream3d"
...
class ForgingClient:
  ...
  def relieve_corrected_forces(self, fx_start, fy_start, fz_start, tx_start, ty_start, tz_start):
    ...
    score_threshold = 0.3
    upper_force_threshold = 70.0
    upper_torque_threshold = 7.0
    distance_increment = .0005
    move_speed = 0.03
    force_score_multiplier = 1.0 / upper_force_threshold
    torque_score_multiplier = 1.0 / upper_torque_threshold
    ft_wait = 0.020
    ...
    while move_success:
      ...
      move_success = False
      dist_increments = [[distance_increment, 0, 0], [0, distance_increment, 0], [0, 0, distance_increment]]

      for dist_increment in dist_increments:
        ...
        move_score1 = force_score_multiplier*((abs(fx0) - abs(fx1)) + (abs(fy0) - abs(fy1)) + (
          abs(fz0) - abs(fz1))) + torque_score_multiplier*((abs(tx0) - abs(tx1)) + (
          abs(ty0) - abs(ty1)) + (abs(tz0) - abs(tz1)))

        print("move score 1 = {}".format(move_score1))

        # Next move 2 dist_increments in positive direction
        dir = 2
        target_pose.position.x += (dir*dist_increment[0])
        target_pose.position.y += (dir*dist_increment[1])
        target_pose.position.z += (dir*dist_increment[2])
        ...
        time.sleep(ft_wait)
        fx2 = copy.deepcopy(ft_sensor.corrected.wrench.force.x)
        fy2 = copy.deepcopy(ft_sensor.corrected.wrench.force.y)
        fz2 = copy.deepcopy(ft_sensor.corrected.wrench.force.z)
        tx2 = copy.deepcopy(ft_sensor.corrected.wrench.torque.x)
        ty2 = copy.deepcopy(ft_sensor.corrected.wrench.torque.y)
        tz2 = copy.deepcopy(ft_sensor.corrected.wrench.torque.z)

        move_score2 = force_score_multiplier*((abs(fx0) - abs(fx2)) + (abs(fy0) - abs(fy2)) + (
          abs(fz0) - abs(fz2))) + torque_score_multiplier*((abs(tx0) - abs(tx2)) + (
          abs(ty0) - abs(ty2)) + (abs(tz0) - abs(tz2)))

        print("move score 2 = {}".format(move_score2))
        
        if move_score2 > score_threshold and move_score2 >= move_score1:
          # The last move was most successful, no other moves needed
          move_success = True
        elif move_score1 > score_threshold and move_score1 > move_score2:
          # The first move (in -x direction) was most successful; move back there
          dir = -2
          target_pose.position.x += (dir*dist_increment[0])
          target_pose.position.y += (dir*dist_increment[1])
          target_pose.position.z += (dir*dist_increment[2])
          ...
          # Neither move was successful; return to starting position
          dir = -1
          target_pose.position.x += (dir*dist_increment[0])
          target_pose.position.y += (dir*dist_increment[1])
          target_pose.position.z += (dir*dist_increment[2])
      ...
      if move_success:
        rospy.loginfo("Relief move succeeded")
